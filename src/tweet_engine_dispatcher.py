...
from tweet_engine_whalealert import tweet_engine_whalealert_helper


def tweet_engine_dispatcher(status):
    if status.user.id_str == ELON_TWITTER_ACCOUNT_ID:
        tweet_engine_elon(status)
    elif status.user.id_str in [DELTAONE_TWITTER_ACCOUNT_ID, FIRSTSQUAWK_TWITTER_ACCOUNT_ID]:
